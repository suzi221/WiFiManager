/*
    ...
    此程序为网页配置WiFi、设备信息和接口信息的代码，启动程序会进入AP模式，默认密码：12345678，连接WiFi后打开网页输入192.168.4.1
    进入配置页面，配置参数后自动重启连接贝壳，连接成功后保存数据，如果连接WiFi失败或者登陆30s贝壳后失败，则重新进入配置模式
    配置完成，上传随机数据
...
char device_id[10]="";// 你的设备编号==
char api_key[20] = "";// 设备密码==
char interface_id[10]="";// 数据接口
#define ONE_WIRE_BUS D2  // DS18B20 温度传感器针脚
...
void setup() {
  ...
  initReadFile();
  initAutoConnect();
  Serial.println("connected ... read config file:\nDEVICEID:"+String(device_id)+",APIKEY:"+String(api_key)+",InterfaceId:"+String(interface_id));
}
WiFiClient client;
void loop() {
  ...
  if(millis() - lastUpdateTime > 6000) {
   ...
   //随机数值30~40
    ...
    update1(String(device_id),String(interface_id),val);
  }
  ...
  Serial.println("mounting FS...");
    ...
    if (SPIFFS.exists(filepath)) {
      ...
      if (configFile) {
        ...
        aJsonObject* interfaceid = aJson.getObjectItem(json1, "interface_id");
        String d_id=id->valuestring;
        String d_key=key->valuestring;
        String i_id=interfaceid->valuestring;
        strcpy(device_id, d_id.c_str());
        strcpy(api_key, d_key.c_str());
        strcpy(interface_id, i_id.c_str());
    configFile.close();
      }
    }
  } else {
...
void initAutoConnect(){
  ...
  WiFiManagerParameter custom_interface_id("interface_id", "\u63a5\u53e3 ID", interface_id, 10);
  ...
  wifiManager.addParameter(&custom_interface_id);
  ...
  strcpy(interface_id, custom_interface_id.getValue());
  ...
  if (shouldSaveConfig) {
    ...
    aJson.addStringToObject(json, "interface_id", interface_id);
...
void checkIn() {
    String msg = "{\"M\":\"checkin\",\"ID\":\"" + String(device_id) + "\",\"K\":\"" + String(api_key) + "\"}\n";
    client.print(msg);
    Serial.println("say to bigiot:"+msg);
    lastCheckInTime = millis(); 
}
...
void checkOut() {
    String msg = "{\"M\":\"checkout\",\"ID\":\"" + String(device_id) + "\",\"K\":\"" + String(api_key) + "\"}\n";
    client.print(msg);
    Serial.println("say to bigiot:"+msg);
}
//{"M":"update","ID":"xx1","V":{"id1":"value1",...}}\n
void update1(String did, String inputid, float value){
  String msg = "{\"M\":\"update\",\"ID\":\"" + did + "\",\"V\":{\"" + inputid + "\":\""+value+"\"}}\n";
  client.print(msg);
  Serial.println("say to bigiot:"+msg);
  lastCheckInTime = millis();
